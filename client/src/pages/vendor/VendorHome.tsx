...
function MapPicker({ lat, lng, onChange, address }: MapPickerProps) {
  ...
  useEffect(() => {
    ...
    const initialLat = lat || 19.076; // Mumbai default
  ...
  useEffect(() => {
    if (mapInstance.current && markerInstance.current) {
      ...
      const targetLat = lat || 19.076;
      const targetLng = lng || 72.8777;

      if (
        Math.abs(currentLatLng.lat - targetLat) > 0.0001 ||
        Math.abs(currentLatLng.lng - targetLng) > 0.0001
      ) {
...
function CustomerLocationModal({ order, onClose }: CustomerLocationModalProps) {
  const mapRef = React.useRef<HTMLDivElement>(null);
  const mapInstance = React.useRef<any>(null);
  const [isLoaded, setIsLoaded] = useState(false);

  // Load Leaflet (already may be loaded by MapPicker)
  useEffect(() => {
    if ((window as any).L) {
      setIsLoaded(true);
      return;
  ...
  useEffect(() => {
    ...
    const lng = order.coordinates?.[0] ?? 72.8777;
    const lat = order.coordinates?.[1] ?? 19.076;
    ...
    const customerIcon = L.divIcon({
      ...
      html: `<div style="position:relative;width:36px;height:36px;display:flex;align-items:center;justify-content:center">
                     <div style="position:absolute;width:16px;height:16px;background:rgba(59,130,246,0.3);border-radius:50%;animation:ripple-ring 1.5s infinite ease-out;top:10px;left:10px"></div>
                     <svg width="36" height="36" viewBox="0 0 24 24" fill="none" style="filter:drop-shadow(0 3px 4px rgba(0,0,0,0.25));position:relative;z-index:10">
    ...
    L.marker([lat, lng], { icon: customerIcon })
      .addTo(map)
      .bindPopup(
        `<strong>${order.firstName} ${order.lastName}</strong><br/>${order.address || ''}`,
      );
  ...
  return (
    <div
      className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-charcoal/60 backdrop-blur-md"
      onClick={onClose}
    >
      <div
        className="bg-white/95 border border-white/30 shadow-[0_24px_70px_rgba(43,33,24,0.3)] rounded-[28px] w-full max-w-md overflow-hidden animate-scale-up"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="bg-gradient-to-br from-[#1a2a4a] to-[#233666] p-5 text-white relative">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-blue-400/20 border border-blue-300/30 flex items-center justify-center shrink-0">
              <svg
        ...
        <div className="p-4">
          {!isLoaded ? (
            <div className="w-full h-52 rounded-2xl bg-charcoal/5 border border-charcoal/10 flex items-center justify-center">
            ...
            <div
              ref={mapRef}
              className="w-full h-52 rounded-2xl border border-charcoal/10 overflow-hidden shadow-inner z-0"
            />
          )}

          {/* Coords + Open in Maps */}
          <div className="mt-3 flex items-center justify-between">
            <div className="text-[10px] text-charcoal/50 font-semibold space-y-0.5">
              <p>
                Lat:{' '}
            ...
            <a
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold transition-all shadow-sm"
            >
              <svg
...
export default function VendorHome({
  ...
  const [locationModalOrder, setLocationModalOrder] = useState<Order | null>(
    null,
  );
  ...
  // Filter for Orders Section
  const [orderFilter, setOrderFilter] = useState<
    'all' | 'pending' | 'completed' | 'delivered' | 'received'
  >('all');
  ...
  // Fetch profile on mount — also captures vendorId for orders fetch
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        if (profileRes.data.data) {
          ...
          // Capture vendorId (Vendor document _id)
          const vid = profile._id as string;
          ...
          try {
            ...
            const ordersRes = await api.get(`/orders/${vid}`);
            if (Array.isArray(ordersRes.data.data)) {
              setOrders(ordersRes.data.data);
            }
          } catch (ordErr) {
            console.error('Failed to fetch orders', ordErr);
          } finally {
      ...
      try {
        ...
        if (menuRes.data.data) {
          const mappedMenu: MenuData = {
            ...menuRes.data.data,
            session: menuRes.data.data.session === 'lunch' ? 'Lunch' : 'Dinner',
          };
          setMenu(mappedMenu);
        } else {
          setMenu(null);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < formTiers.length; i++) {
      ...
      if (t.items.length === 0) {
        setFormError(`Tier "${t.name}" must have at least one menu item tag.`);
        return;
      }
      ...
      if (t.maxQuantity === '' || Number(t.maxQuantity) <= 0) {
        setFormError(`Tier "${t.name}" must have a valid maximum capacity.`);
        return;
      }
    }
    ...
    for (let i = 0; i < formAddOns.length; i++) {
      ...
      if (!ad.name.trim() || ad.price === '' || Number(ad.price) <= 0) {
        setFormError(`Add-on row ${i + 1} must have a name and a valid price.`);
        return;
      }
    }
    ...
    try {
      ...
      setFormError(err.response?.data?.message || 'Failed to upload menu');
    } finally {
  ...
  // Accept Order — wired to backend API: PUT /api/orders/:orderId/accept
  const handleAcceptOrder = async (orderId: string) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((ord) =>
          ord._id === orderId ? { ...ord, status: 'accepted' } : ord,
        ),
      );
    } catch (err: any) {
  ...
  const handleRejectOrder = async (orderId: string) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((ord) =>
          ord._id === orderId ? { ...ord, status: 'rejected' } : ord,
        ),
      );
    } catch (err: any) {
  ...
  const handleDeliverOrder = async (orderId: string) => {
    ...
    try {
      ...
      setOrderActionError(
        err.response?.data?.message ||
          'Failed to deliver order. Please try again.',
      );
    } finally {
  ...
  const filteredOrders = orders.filter((ord) => {
    ...
    if (orderFilter === 'received') return ord.status === 'received';
    return true;
  });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          >
            ...
            {menuLoading ? (
              ...
              <div className="bg-white/40 border border-white/30 backdrop-blur-xl shadow-[0_24px_70px_-15px_rgba(43,33,24,0.12)] rounded-[32px] p-6 relative overflow-hidden transition-all duration-300">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center space-x-2">
                    ...
                    {!isOpen && (
                      <span
                        title="Tier details (name, price, items, capacity) can't be edited once an order has been placed against this menu. You can still add new tiers or add-ons."
                        style={{ cursor: 'help' }}
                        className="px-3 py-1 rounded-full bg-charcoal/10 border border-charcoal/20 text-charcoal/60 text-xs font-bold uppercase tracking-wider flex items-center gap-1 select-none"
                      >
                        <svg
                          className="w-3 h-3"
                          fill="none"
                    ...
                    >
                      ...
                      <div className="bg-charcoal/5 p-3 rounded-xl border border-charcoal/5">
                        <div className="flex justify-between items-center text-[10px] font-bold mb-1 font-body">
                          ...
                          >
                            {tier.remainingQuantity} / {tier.maxQuantity} Left
                          </span>
                        </div>
                ...
                {menu.addOns && menu.addOns.length > 0 && (
                  <div className="mb-6 border-t border-charcoal/10 pt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {menu.addOns.map((addOn, addOnIdx) => (
                        <span
                          key={addOnIdx}
              ...
              <div
                className={`bg-white/30 backdrop-blur-md border-2 border-dashed rounded-[32px] p-8 text-center flex flex-col items-center justify-center min-h-[400px] shadow-sm transition-all duration-300 group relative ${
                  isOpen
                    ? 'border-charcoal/10 hover:border-spice/30'
                    : 'border-charcoal/20 opacity-70'
                }`}
              >
                {!isOpen && (
                ...
                <div className="w-24 h-24 mb-6 text-charcoal/30 stroke-current group-hover:text-spice/55 transition-colors duration-300">
                  ...
                  >
                    ...
                    {/* Rivet Details */}
                    <circle cx="27" cy="68" r="1.5" fill="currentColor" />
                    <circle cx="73" cy="68" r="1.5" fill="currentColor" />
                    <circle cx="27" cy="92" r="1.5" fill="currentColor" />
                    <circle cx="73" cy="92" r="1.5" fill="currentColor" />
                  </svg>
                </div>
                ...
                <p className="text-xs text-charcoal/50 max-w-xs leading-relaxed mb-6 font-body">
                  Upload today's dishes to populate your storefront and activate
                  your subscription orders pipeline.
                </p>
                <button
                  disabled={!isOpen}
                  onClick={() => {
          ...
          {/* ==========================================
                       ...
                       ========================================== */}
          <div
            className={`lg:col-span-7 space-y-6 transition-all duration-300 ${!isOpen ? 'opacity-65 grayscale-[35%]' : ''}`}
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-1">
              ...
              <div className="flex flex-wrap border-b border-charcoal/10 w-full sm:w-auto relative mb-1 shrink-0 select-none">
                {(
                  [
                  ...
                  const isActive = orderFilter === key;
            ...
            {orderActionError && (
              <div className="mb-4 p-3 rounded-2xl bg-spice/10 border border-spice/20 text-spice text-xs flex items-start justify-between gap-2 font-semibold">
                ...
                <button
                  onClick={() => setOrderActionError(null)}
                  className="shrink-0 text-spice/70 hover:text-spice ml-1"
                >
                  <svg
                    className="w-3.5 h-3.5"
                    fill="none"
            ...
            {profileLoading || ordersLoading ? (
              ...
              <div className="bg-white/30 backdrop-blur-md border border-charcoal/10 border-dashed rounded-[32px] p-8 text-center flex flex-col items-center justify-center min-h-[360px] shadow-sm">
                ...
                <p className="text-xs text-charcoal/50 max-w-xs leading-relaxed font-body">
                  Subscriptions and on-demand orders matching this filter will
                  stream here in real time.
                </p>
              </div>
            ) : (
              /* Orders Pipeline List with staggered fade-in animations */
              <div className="max-h-[500px] overflow-y-auto pr-3 space-y-6 custom-scrollbar">
                {filteredOrders.map((order, index) => {
                  ...
                  let timeColorClass = 'text-charcoal/40 font-semibold';

                  if (diffMin > 0) {
                    if (diffMin < 60) timeAgoText = `${diffMin} min ago`;
                    else if (diffMin < 1440)
                      timeAgoText = `${Math.floor(diffMin / 60)} hr ago`;
                    else timeAgoText = 'Older';
                  }

                  if (diffMin >= 30) {
                    timeColorClass = 'text-red-500 font-bold opacity-90';
                  } else if (diffMin >= 15) {
                    timeColorClass = 'text-amber-500 font-bold opacity-90';
                  }
                  ...
                  return (
                    <div
                      ...
                      className="bg-[#faf6f0] border border-charcoal/5 shadow-sm hover:shadow-md rounded-[24px] p-6 transition-all relative flex flex-col gap-3 animate-fade-in-up"
                    >
                      {/* Top Row: Label & Price */}
                      ...
                      <div className="flex flex-col gap-2 mt-1">
                        <div className="flex flex-wrap items-center gap-2.5">
                          <span
                            className={`px-3 py-1 rounded-md text-[10px] font-bold border shadow-[0_1px_2px_rgba(0,0,0,0.02)] ${badgeStyle}`}
                          >
                            {badgeText}
                          </span>
                          {order.addOns &&
                      ...
                      {/* Bottom Info & Actions (Lighter, bottom-right actions) */}
                      <div className="flex items-end justify-between mt-3 pt-3 border-t border-charcoal/5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center text-[10px] font-black text-charcoal/50 border border-charcoal/5">
                            {getInitials(
                              `${order.firstName} ${order.lastName}`,
                            )}
                          </div>
                          <div className="flex flex-col">
                            <div className="flex items-center gap-1">
                              <span className="text-xs font-semibold text-charcoal/60 truncate max-w-[120px]">
                                {order.firstName} {order.lastName}
                              </span>
                              {order.coordinates && (
                                <button
                                  onClick={() => setLocationModalOrder(order)}
                                  className="text-charcoal/40 hover:text-charcoal transition-colors ml-1"
                        ...
                        <div className="flex items-center justify-end gap-2 min-w-[120px]">
                          {order.status === 'pending' && (
                            <>
                              <button
                                disabled={
                                  !isOpen || rejectingOrderId === order._id
                                }
                                onClick={() => handleRejectOrder(order._id)}
                                className="px-3 py-2 text-xs font-bold text-charcoal/50 hover:text-red-500 hover:bg-charcoal/5 rounded-lg transition-all disabled:opacity-50"
                              >
                              ...
                              <button
                                disabled={
                                  !isOpen || acceptingOrderId === order._id
                                }
                                onClick={() => handleAcceptOrder(order._id)}
                                className="px-4 py-2 text-xs font-bold text-white bg-spice hover:bg-[#E0592E] rounded-lg transition-all shadow-sm disabled:opacity-50"
                              >
                          ...
                          {order.status === 'accepted' && (
                            <button
                              ...
                              className="px-4 py-2 text-xs font-bold text-white bg-green-600 hover:bg-green-700 rounded-lg transition-all shadow-sm disabled:opacity-50"
                            >
            ...
            <div className="bg-gradient-to-br from-[#1F1710] to-[#2E2218] p-6 text-cream relative select-none">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-cream/60 mt-1">
                  Specify tiers, add-ons, pricing, and stock capacities for
                  today's session. The session (Lunch/Dinner) is detected
            ...
            >
                ...
                <div className="flex bg-charcoal/5 p-1 rounded-xl border border-charcoal/10 w-60 relative select-none">
                  {(['Lunch', 'Dinner'] as const).map((session) => (
                    <button
                      key={session}
                      type="button"
                      onClick={() => setFormSession(session)}
                  ...
                  >
                    ...
                    <div className="relative w-full group">
                      <input
                        ...
                        onFocus={() => setFocusedField(`tier-${index}-name`)}
                      ...
                      <div className="border border-charcoal/15 focus-within:border-spice focus-within:ring-2 focus-within:ring-spice/20 rounded-2xl p-3 bg-[#FBF4EC]/50 transition-all min-h-[80px] flex flex-wrap gap-2 items-start content-start">
                        {tier.items.map((item, tagIdx) => (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleRemoveTierTag(index, tagIdx)}
                        ...
                        <input
                          ...
                          onKeyDown={(e) => handleTierKeyDown(index, e)}
                          placeholder={
                            tier.items.length === 0
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="relative w-full group">
                        <input
                          ...
                          onFocus={() => setFocusedField(`tier-${index}-price`)}
                          onBlur={() => setFocusedField(null)}
                          className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                          placeholder="Price (₹)"
                        />
                      ...
                      <div className="relative w-full group">
                        <input
                          ...
                          onFocus={() => setFocusedField(`tier-${index}-max`)}
                          onBlur={() => setFocusedField(null)}
                          className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                  ...
                  >
                    ...
                    <div className="relative flex-1 group">
                      <input
                        ...
                        onFocus={() => setFocusedField(`addon-${index}-name`)}
                        onBlur={() => setFocusedField(null)}
                    ...
                    <div className="relative w-24 group">
                      <input
                        ...
                        onFocus={() => setFocusedField(`addon-${index}-price`)}
                        onBlur={() => setFocusedField(null)}
                        className="w-full bg-[#FBF4EC]/60 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                        placeholder="10"
                      />
                      <label
                        className={`absolute left-5 transition-all duration-300 pointer-events-none font-body ${
                          focusedField === `addon-${index}-price` ||
                          addOn.price !== ''
                            ? 'top-2 text-[10px] font-bold uppercase tracking-wider text-spice'
                            : 'top-4 text-sm text-charcoal/60'
                        }`}
              ...
              <div className="pt-2 flex gap-3">
                ...
                >
                  ...
                  <span>
                    {formSubmitting ? 'Uploading...' : 'Upload & Launch'}
                  </span>
                </button>
              </div>
            </form>
          ...
          >
            ...
            <div className="bg-gradient-to-br from-[#1F1710] to-[#2E2218] p-6 text-cream relative select-none">
              ...
              <div className="relative z-10">
                ...
                <p className="text-xs text-cream/60 mt-1">
                  Update your personal information, business details, and
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="relative w-full group">
                    <input
                      ...
                      onFocus={() => setFocusedField('profile-firstname')}
                      onBlur={() => setFocusedField(null)}
                      className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                    ...
                    <label
                      className={`absolute left-5 transition-all duration-300 pointer-events-none font-body ${
                        focusedField === 'profile-firstname' ||
                        formFirstname !== ''
                          ? 'top-2 text-[10px] font-bold uppercase tracking-wider text-spice'
                          : 'top-4 text-sm text-charcoal/60'
                      }`}
                  ...
                  <div className="relative w-full group">
                    <input
                      ...
                      onFocus={() => setFocusedField('profile-lastname')}
                      onBlur={() => setFocusedField(null)}
                      className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="relative w-full group">
                    <input
                      ...
                      onFocus={() => setFocusedField('profile-email')}
                      onBlur={() => setFocusedField(null)}
                      className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                    ...
                    <label
                      className={`absolute left-5 transition-all duration-300 pointer-events-none font-body ${
                        focusedField === 'profile-email' || formEmail !== ''
                          ? 'top-2 text-[10px] font-bold uppercase tracking-wider text-spice'
                          : 'top-4 text-sm text-charcoal/60'
                      }`}
                    >
                  ...
                  <div className="relative w-full group">
                    ...
                    <label
                      className={`absolute left-5 transition-all duration-300 pointer-events-none font-body ${
                        focusedField === 'profile-phone' || formPhone !== ''
                          ? 'top-2 text-[10px] font-bold uppercase tracking-wider text-spice'
                          : 'top-4 text-sm text-charcoal/60'
                      }`}
                    >
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="relative w-full group sm:col-span-2">
                    <input
                      ...
                      onFocus={() => setFocusedField('profile-bizname')}
                      onBlur={() => setFocusedField(null)}
                      className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                  ...
                  <div className="relative w-full group">
                    <input
                      ...
                      onFocus={() => setFocusedField('profile-radius')}
                      onBlur={() => setFocusedField(null)}
                      className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                    ...
                    <label
                      className={`absolute left-5 transition-all duration-300 pointer-events-none font-body ${
                        focusedField === 'profile-radius' || formRadius !== ''
                          ? 'top-2 text-[10px] font-bold uppercase tracking-wider text-spice'
                          : 'top-4 text-sm text-charcoal/60'
                      }`}
                    >
                ...
                <div className="relative w-full group">
                  <input
                    ...
                    onFocus={() => setFocusedField('profile-address')}
                    onBlur={() => setFocusedField(null)}
                    className="w-full bg-[#FBF4EC]/70 border border-charcoal/15 text-charcoal rounded-2xl px-5 pt-6 pb-2 text-sm focus:outline-none focus:ring-2 focus:border-spice focus:ring-spice/20 transition-all font-body select-text placeholder-transparent focus:placeholder-charcoal/30"
                  ...
                  <label
                    className={`absolute left-5 transition-all duration-300 pointer-events-none font-body ${
                      focusedField === 'profile-address' || formAddress !== ''
                        ? 'top-2 text-[10px] font-bold uppercase tracking-wider text-spice'
                        : 'top-4 text-sm text-charcoal/60'
                    }`}
                  >
