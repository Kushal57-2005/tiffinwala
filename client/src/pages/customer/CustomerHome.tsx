...
export default function CustomerHome({
  ...
  const handleAddMoney = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userId = authStoreState.user?._id || customerIdLocal;

      if (!userId) {
        setAddMoneyError('User session not found. Please log in again.');
        setIsProcessingPayment(false);
        return;
      }
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            const verifyRes = await api.post('/payments/verify-wallet', {
              userId,
              amount: amountNum,
            ...
            setAddMoneyError(
              verifyErr.response?.data?.message || 'Verification failed.',
            );
          }
        },
        prefill: {
      ...
      rzp.on('payment.failed', function (response: any) {
        setAddMoneyError(response.error.description || 'Payment failed.');
      });
      rzp.open();
    } catch (err: any) {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (res.data.success && res.data.data) {
          ...
          if (profile.userId) {
            setCustomerIdLocal(profile.userId._id || profile.userId);
          }
  ...
  useEffect(() => {
    const fetchTodayOrders = async () => {
      try {
        ...
        const res = await api.get('/customer/my-today');
  ...
  const handleReceiveOrder = async (orderId: string) => {
    ...
    try {
      ...
      setTodayOrders((prev) =>
        prev.map((ord) =>
          ord._id === orderId ? { ...ord, status: 'received' } : ord,
        ),
      );
    } catch (err) {
      ...
      const fetchNearby = async () => {
        try {
          ...
          if (err.response?.status === 400) {
            setLocationError(
              err.response?.data?.message || 'Please set your location first.',
            );
          } else {
            setLocationError(
  ...
  const handleOpenDetails = async (vendor: Vendor) => {
    ...
    try {
      ...
      if (
        ...
        menuRes.value.data.success &&
        menuRes.value.data.data
      ) {
        ...
        <div className="max-w-6xl mx-auto px-4 pt-6 md:pt-10 relative z-10">
          ...
          <div className="mt-2 mb-8 flex flex-wrap gap-4 items-center justify-start bg-amber-500/5 border border-dashed border-amber-500/35 backdrop-blur-md px-5 py-3.5 rounded-2xl text-xs font-semibold text-[#2B2118]/70 shadow-sm relative z-20">
            ...
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                ...
                onChange={(e) => setSimulateEmptyMyVendors(e.target.checked)}
            ...
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                ...
                onChange={(e) => setSimulateEmptyNearby(e.target.checked)}
                className="rounded border-amber-500/30 text-amber-600 focus:ring-amber-500/40"
              />
          ...
          <section className="mt-8 mb-12 max-w-2xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-3.5 px-1 select-none">
              {['Open Now', 'Top Rated', 'Within 2 km'].map((filter) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              {loading ? (
                ...
                <div className="bg-white/30 backdrop-blur-md border border-[#2B2118]/10 border-dashed rounded-[32px] p-8 text-center flex flex-col items-center justify-center min-h-[180px] shadow-sm transition-all duration-300">
                  ...
                  <p className="text-xs text-[#2B2118]/50 max-w-xs leading-relaxed font-body">
                    Subscribe to standard daily meal packages or place one-time
                    orders to lock in your vendor shortcuts.
                  </p>
                </div>
              ) : (
            ...
            {(todayOrders.length > 0 || ordersLoading) && (
              <section className="lg:col-span-12 space-y-4 pt-4">
                <div className="px-1 flex items-center justify-between">
                  <h2 className="font-display text-2xl font-bold text-charcoal flex items-center space-x-2">
                ...
                {ordersLoading ? (
                  <div className="flex gap-4 overflow-x-auto pb-3 pt-1 animate-pulse">
                    {[1, 2, 3].map((n) => (
                      <div
                        key={n}
                        className="flex-shrink-0 bg-white/20 border border-white/10 rounded-2xl p-4 flex items-center space-x-3 w-64 h-28 shadow-sm"
                      >
                        <div className="space-y-3 flex-1">
                          <div className="h-4 w-1/3 bg-charcoal/10 rounded" />
                          <div className="h-5 w-3/4 bg-charcoal/10 rounded" />
                          <div className="h-3 w-1/2 bg-charcoal/10 rounded" />
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="flex gap-5 overflow-x-auto pb-4 pt-2 custom-scrollbar scroll-smooth px-1">
                    {todayOrders.map((order) => {
                      const diffMin = Math.floor(
                        (new Date().getTime() -
                          new Date(
                            order.orderTime || (order as any).createdAt || new Date(),
                          ).getTime()) /
                          60000,
                      );
                      ...
                      return (
                        <div
                          key={order._id}
                          className={`flex-shrink-0 bg-[#faf6f0] border border-charcoal/5 shadow-sm rounded-[24px] p-6 w-[340px] flex flex-col justify-between hover:shadow-md transition-all duration-300 text-left ${extraStyle}`}
                        >
                          <div className="flex flex-col gap-3">
                            ...
                            <div className="flex items-end justify-between mt-3 pt-3 border-t border-charcoal/5">
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center text-charcoal/40 border border-charcoal/5">
                                  <svg
                                    className="w-4 h-4"
                                    fill="none"
                                    stroke="currentColor"
                                ...
                                <div className="flex flex-col">
                                  <span className="text-xs font-semibold text-charcoal/60 truncate max-w-[120px]">
                                    {order.businessName || 'Vendor'}
                                  </span>
                                  <span className="text-[9px] font-black text-charcoal/40 uppercase tracking-widest mt-0.5">
                                    {order.session} SESSION
                                  </span>
                                </div>
                              </div>

                              {/* Action Button Area (always occupies space for alignment) */}
                              <div className="flex justify-end min-w-[120px]">
                                {order.status === 'delivered' && (
                                  <button
                                    disabled={receivingOrderId === order._id}
                                    onClick={() =>
                                      handleReceiveOrder(order._id)
                                    }
                                    className={`px-4 py-2 text-xs font-bold rounded-lg transition-all shadow-sm text-white bg-charcoal hover:bg-charcoal/90 ${receivingOrderId === order._id ? 'opacity-70 cursor-wait' : ''}`}
                                  >
            ...
            <section className="lg:col-span-12 space-y-5 pt-4">
              ...
              {locationError ? (
                ...
                <div className="bg-white/30 backdrop-blur-md border border-[#2B2118]/10 border-dashed rounded-[32px] p-12 text-center flex flex-col items-center justify-center min-h-[350px] shadow-sm transition-all duration-300">
                  ...
                  <p className="text-xs text-[#2B2118]/50 max-w-sm leading-relaxed mb-6 font-body">
                    We couldn't locate any active tiffin kitchens matching your
                    filters. Try adjusting your coordinates or check back later
                    as kitchens onboard!
                  </p>
                  <button
                    onClick={() => {
                  ...
                  <div>
                    <div className="flex gap-1.5 items-center mb-1.5 flex-wrap">
                      ...
                      <span className="px-2 py-0.5 rounded-full bg-white/10 text-cream/70 text-[9px] font-bold">
                        {selectedVendor.distanceKm} km away
                      </span>
                      <span className="px-2 py-0.5 rounded-full bg-white/10 text-cream/70 text-[9px] font-bold">
                        {selectedVendor.deliveryRadiusKm} km delivery radius
                      </span>
                    </div>
                    ...
                    <p className="text-xs text-cream/60 mt-0.5">
                      Kitchen Lead: <strong>{selectedVendor.ownerName}</strong>
                    </p>
              ...
              {orderSuccess ? (
                ...
                <div className="p-8 text-center flex flex-col items-center justify-center min-h-[350px] bg-[#5C7A52]/5 transition-all">
                  ...
                  <p className="text-xs text-[#2B2118]/60 max-w-xs leading-relaxed font-body">
                    ...
                    <strong>{selectedVendor.businessName}</strong>. You will be
                ...
                >
                  ...
                  {/* Error Message */}
                  {orderError && (
                    <div className="mt-4 p-3 rounded-xl bg-[#E0653A]/15 border border-[#E0653A]/20 text-[#E0653A] text-xs flex items-start space-x-2 font-body animate-pulse">
                      <svg
                  ...
                  <div className="border-t border-[#2B2118]/10 mt-4 pt-4 flex items-center justify-between gap-4">
                    <div className="text-left font-body">
              ...
              <div className="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p className="text-[#FBF4EC]/60 text-[11px] font-body leading-relaxed max-w-[280px]">
                    Manage your personal information, delivery address, and
                    friends.
                  </p>
                </div>
                <button
                ...
                <form
                  ...
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      setCustomerName(fullName);
                      setCustomerAddress(
                        updated.location?.address || formAddress,
                      );
                    ...
                    friendFormMode === null && (
                      <div className="flex flex-col items-center py-10 gap-3">
                        ...
                        <p className="text-xs text-[#2B2118]/30 font-body text-center">
                          Add a friend to split tiffin orders!
                        </p>
                        <button
                          onClick={() => {
                            ...
                            <div className="bg-white/60 border border-white/50 rounded-2xl px-4 py-3.5 shadow-sm">
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                  ...
                                  <button
                                    onClick={() => {
                                      setFriendFormMode('edit');
                                      setEditingFriendId(friend._id);
                                      setFriendFormName(friend.name);
                                      setFriendFormNickname(
                                        friend.nickname || '',
                                      );
                                      setFriendFormPhone(friend.phone || '');
                                      setFriendFormError('');
                                      setDeletingFriendId(null);
                                    }}
                                  ...
                                  <button
                                    onClick={() =>
                                      setDeletingFriendId((prev) =>
                                        prev === friend._id ? null : friend._id,
                                      )
                                    }
                                    ...
                                    <button
                                      onClick={async () => {
                                        try {
                                          ...
                                          setFriendsLoading(true);
                                          const r =
                                            await api.get('/customer/friends');
                                          setFriends(
                                            r.data.data?.friendProfiles ||
                                              r.data.data ||
                                              [],
                                          );
                                        } catch {
                              ...
                              <form
                                ...
                                onSubmit={async (e) => {
                                  ...
                                  if (!friendFormName.trim()) {
                                    setFriendFormError('Name is required');
                                    return;
                                  }
                              ...
                              >
                                ...
                                <div className="relative w-full">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setFriendFormName(e.target.value)
                                    }
                                    onFocus={() => setFriendFocused('ef-name')}
                                    onBlur={() => setFriendFocused(null)}
                                ...
                                <div className="relative w-full">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setFriendFormNickname(e.target.value)
                                    }
                                    onFocus={() => setFriendFocused('ef-nick')}
                                    onBlur={() => setFriendFocused(null)}
                                    className="w-full bg-[#FBF4EC]/80 border border-[#2B2118]/15 text-[#2B2118] rounded-xl px-4 pt-5 pb-1.5 text-sm focus:outline-none focus:ring-2 focus:border-[#5C7A52] focus:ring-[#5C7A52]/20 transition-all font-body placeholder-transparent"
                                ...
                                <div className="relative w-full">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setFriendFormPhone(e.target.value)
                                    }
                                    onFocus={() => setFriendFocused('ef-phone')}
                                    onBlur={() => setFriendFocused(null)}
                                    className="w-full bg-[#FBF4EC]/80 border border-[#2B2118]/15 text-[#2B2118] rounded-xl px-4 pt-5 pb-1.5 text-sm focus:outline-none focus:ring-2 focus:border-[#5C7A52] focus:ring-[#5C7A52]/20 transition-all font-body placeholder-transparent"
                                    placeholder="Phone"
                                  />
                  ...
                  {friendFormMode === 'add' && (
                    <form
                      ...
                      onSubmit={async (e) => {
                        ...
                        setFriendFormError('');
                        try {
                          await api.post('/customer/friends', {
                            name: friendFormName,
                            nickname: friendFormNickname,
                            phone: friendFormPhone,
                          });
                    ...
                    >
                      ...
                      <div className="relative w-full">
                        <input
                          type="text"
                          value={friendFormName}
                          onChange={(e) => setFriendFormName(e.target.value)}
                          onFocus={() => setFriendFocused('af-name')}
                          onBlur={() => setFriendFocused(null)}
                          className="w-full bg-[#FBF4EC]/80 border border-[#2B2118]/15 text-[#2B2118] rounded-xl px-4 pt-5 pb-1.5 text-sm focus:outline-none focus:ring-2 focus:border-[#5C7A52] focus:ring-[#5C7A52]/20 transition-all font-body placeholder-transparent"
                          placeholder="Name"
                          autoFocus
                      ...
                      <div className="relative w-full">
                        <input
                          ...
                          onFocus={() => setFriendFocused('af-nick')}
                          onBlur={() => setFriendFocused(null)}
                          className="w-full bg-[#FBF4EC]/80 border border-[#2B2118]/15 text-[#2B2118] rounded-xl px-4 pt-5 pb-1.5 text-sm focus:outline-none focus:ring-2 focus:border-[#5C7A52] focus:ring-[#5C7A52]/20 transition-all font-body placeholder-transparent"
                          placeholder="Nickname"
                        />
                      ...
                      <div className="relative w-full">
                        <input
                          type="text"
                          value={friendFormPhone}
                          onChange={(e) => setFriendFormPhone(e.target.value)}
                          onFocus={() => setFriendFocused('af-phone')}
                          onBlur={() => setFriendFocused(null)}
                          className="w-full bg-[#FBF4EC]/80 border border-[#2B2118]/15 text-[#2B2118] rounded-xl px-4 pt-5 pb-1.5 text-sm focus:outline-none focus:ring-2 focus:border-[#5C7A52] focus:ring-[#5C7A52]/20 transition-all font-body placeholder-transparent"
                          placeholder="Phone"
                        />
                      ...
                      <div className="flex gap-2 pt-1">
                        <button
                          type="button"
                          onClick={() => setFriendFormMode(null)}
                          className="flex-1 py-2.5 rounded-xl border border-[#2B2118]/15 text-[#2B2118]/60 hover:text-[#2B2118] hover:bg-[#2B2118]/5 text-xs font-bold font-body transition-all"
                        >
                          Cancel
                        </button>
              ...
              <form onSubmit={handleAddMoney} className="space-y-4">
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <span className="text-[#5C7A52] font-display text-xl font-bold">
                      ₹
                    </span>
                  </div>
                  <input
                    ...
                    step="1"
                    autoFocus
                    className="w-full bg-white border border-[#2B2118]/15 text-[#2B2118] rounded-2xl pl-10 pr-4 py-4 font-body font-bold text-lg focus:outline-none focus:ring-2 focus:border-[#5C7A52] focus:ring-[#5C7A52]/20 transition-all placeholder:font-normal placeholder:text-[#2B2118]/30"
                  />
                </div>
                ...
                <div className="flex gap-2">
                  {[500, 1000, 2000].map((amt) => (
                    <button
                      ...
                      className="flex-1 py-2 rounded-xl bg-white border border-[#2B2118]/10 text-[#2B2118]/80 hover:bg-[#5C7A52]/5 hover:border-[#5C7A52]/30 hover:text-[#5C7A52] text-sm font-bold font-body transition-all"
                    >
                      +₹{amt}
                    </button>
                  ))}
                ...
                <button
                  ...
                  className="w-full py-4 mt-2 rounded-2xl bg-[#5C7A52] hover:bg-[#4a6342] text-white font-bold font-body text-base shadow-[0_8px_20px_rgba(92,122,82,0.25)] hover:shadow-[0_10px_25px_rgba(92,122,82,0.35)] hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 flex justify-center items-center gap-2"
                >
                  {isProcessingPayment ? (
                    <>
                      <svg
