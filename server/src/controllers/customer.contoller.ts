    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          result,
          result ? 'Menu fetched successfully' : 'No menu uploaded for today',
        ),
      );
  },
);
...
export const addFriendProfile = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const result = await addFriendProfileService(userId, name, phone, nickname);
    return res
      .status(201)
...
export const getOrderForCustomer = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const userId = req.user?.userId as string;
    const result = await getOrderForCustomerService(userId);
